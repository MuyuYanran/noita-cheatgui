...
| `noita-cheatgui/` | CheatGUI 作弊菜单模组 —— 中文分支 v1.7.3（见下方详述）。 |
...
├── data/hax/
│   ├── cheatgui.lua               # 主 GUI 文件（~2500 行）
│   ├── config.lua                 # 持久化配置系统
│   ├── console.lua                # WebSocket 远程控制台
│   ├── entity_viewer.lua          # 🆕 实体查看器
│   ├── fog_of_war.lua             # 🆕 战争迷雾控制
│   ├── free_camera.lua            # 🆕 自由视角
│   ├── i18n.lua                   # 国际化支持（中/英）
│   ├── alchemy.lua                # 炼金术配方数据
│   ├── fungal.lua                 # 真菌转化数据
│   ├── gun_builder.lua            # 法杖构建器
│   ├── materials.lua              # 材料数据
│   ├── material_converter.lua     # 🆕 材料转化器
│   ├── mod_info.lua               # 🆕 模组信息
│   ├── physics_tweaks.lua         # 🆕 物理参数调整
│   ├── spawnables.lua             # 可生成实体列表
│   ├── special_spawnables.lua     # 特殊可生成实体
│   ├── status_effects.lua         # 🆕 状态效果管理
...
`noita-cheatgui` 是一个功能全面的游戏内作弊/调试菜单模组，基于 [probable-basilisk/cheatgui](https://github.com/probable-basilisk/cheatgui) 的中文分支（v1.7.3）。
...
| **生命** | 获取当前生命值，修改最大生命值，快速 +25/+100/×10/×100 最大生命。 |
...
| **物品** | 生成任意游戏物品和实体。 |
| **状态效果** | 🆕 查看当前状态效果，添加/移除/设置持续时间（60+ 效果），一键清除负面效果。 |
| **自由视角** | 🆕 解锁/锁定摄像机自由移动，方向键移动，可调速度，一键跳转到关键地点。 |
| **实体查看器** | 🆕 通过鼠标/标签/玩家选择实体，展开查看组件成员变量，启用/禁用组件。 |
| **材料转化器** | 🆕 全局/区域材料转化，预设方案（水→金等），"万物成金"，实体→材料，可调半径。 |
| **物理参数** | 🆕 调整重力（月球/火星/零重力/反转）和移动速度倍率，支持自定义数值输入（±步进）。 |
| **战争迷雾** | 🆕 全图揭示/恢复迷雾，区域揭示/恢复/半透明，调整玩家探索半径。区域操作不会超出当前已加载区块范围。 |
| **模组信息** | 🆕 显示活动模组列表、API 版本、构建类型。 |
